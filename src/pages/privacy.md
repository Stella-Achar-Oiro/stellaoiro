---
layout: layouts/page.njk
title: Privacy Policy
description: Privacy Policy for acharoiro.com
permalink: /privacy/
---

**Last Updated: November 17, 2025**

## Introduction

This Privacy Policy explains how Achar Oiro ("we," "us," or "our") collects, uses, and protects your personal information when you visit our website acharoiro.com (the "Site").

We are committed to protecting your privacy and ensuring you understand what information we collect and how we use it.

## Information We Collect

### Information You Provide

When you interact with our Site, you may provide us with:

- **Newsletter Subscription**: Email address
- **Contact Forms**: Name, email address, company, project description
- **Hire Me Form**: Name, email, company, service needed, budget, timeline
- **Comments** (if enabled): Name, email address, comment content

### Information Collected Automatically

When you visit our Site, we may automatically collect:

- **Analytics Data**: Pages visited, time spent on pages, browser type, device type, IP address
- **Cookies**: Small text files stored on your device (see Cookies section below)
- **Log Data**: IP address, browser type, referring URLs, pages visited

## How We Use Your Information

We use the information we collect to:

### Provide Services
- Deliver newsletter content to subscribers
- Respond to inquiries and consultation requests
- Provide technical support and customer service

### Improve Our Site
- Analyze site usage and performance
- Understand user behavior and preferences
- Improve content and user experience

### Communicate
- Send you newsletters (with your consent)
- Respond to your inquiries
- Send important updates about our services

### Legal Compliance
- Comply with applicable laws and regulations
- Protect our rights and safety
- Prevent fraud and abuse

## Third-Party Services

We use the following third-party services that may collect information:

### Google Analytics
- **Purpose**: Website analytics
- **Data Collected**: IP address, pages visited, device type, browser type
- **Privacy Policy**: [Google Analytics Privacy Policy](https://policies.google.com/privacy)

### Newsletter Service (ConvertKit/Mailchimp)
- **Purpose**: Email newsletter delivery
- **Data Collected**: Email address, name, subscription preferences
- **Privacy Policy**: [ConvertKit Privacy Policy](https://convertkit.com/privacy) or [Mailchimp Privacy Policy](https://mailchimp.com/legal/privacy/)

### Form Handling (Netlify Forms)
- **Purpose**: Contact and hire me form submissions
- **Data Collected**: Name, email, form responses
- **Privacy Policy**: [Netlify Privacy Policy](https://www.netlify.com/privacy/)

### AWS (Hosting)
- **Purpose**: Website hosting and content delivery
- **Data Collected**: IP address, access logs
- **Privacy Policy**: [AWS Privacy Policy](https://aws.amazon.com/privacy/)

## Cookies

### What Are Cookies?

Cookies are small text files stored on your device when you visit websites. They help us improve your experience and understand how you use our Site.

### Types of Cookies We Use

**Essential Cookies**
- Required for site functionality
- Remember your preferences
- Cannot be disabled

**Analytics Cookies**
- Help us understand site usage
- Track page views and user behavior
- Can be disabled in your browser

**Advertising Cookies** (if applicable)
- Used for affiliate marketing
- Track conversions from affiliate links
- Can be disabled in your browser

### Managing Cookies

You can control cookies through your browser settings:
- [Chrome Cookie Settings](https://support.google.com/chrome/answer/95647)
- [Firefox Cookie Settings](https://support.mozilla.org/en-US/kb/cookies-information-websites-store-on-your-computer)
- [Safari Cookie Settings](https://support.apple.com/guide/safari/manage-cookies-sfri11471/mac)

Note: Disabling cookies may affect site functionality.

## Your Rights

Depending on your location, you may have the following rights:

### Access
Request a copy of the personal information we hold about you

### Correction
Request correction of inaccurate or incomplete information

### Deletion
Request deletion of your personal information

### Objection
Object to processing of your personal information

### Portability
Request transfer of your data to another service

### Withdrawal of Consent
Withdraw consent for data processing at any time

To exercise these rights, contact us at: stellaacharoiro@gmail.com

## Data Security

We implement reasonable security measures to protect your personal information:

- **Encryption**: HTTPS/SSL encryption for data in transit
- **Access Controls**: Limited access to personal information
- **Secure Storage**: Data stored on secure servers

However, no method of transmission over the Internet is 100% secure. While we strive to protect your information, we cannot guarantee absolute security.

## Data Retention

We retain your personal information for as long as necessary to:

- Provide services you've requested
- Comply with legal obligations
- Resolve disputes and enforce agreements

**Newsletter Data**: Until you unsubscribe
**Contact Form Data**: 2 years after submission
**Analytics Data**: 26 months (Google Analytics default)

## Children's Privacy

Our Site is not intended for children under 13 years of age. We do not knowingly collect personal information from children under 13. If you believe we have collected information from a child under 13, please contact us immediately.

## International Data Transfers

If you are located outside the United States, please note that we may transfer your information to the United States and other countries. By using our Site, you consent to this transfer.

## Affiliate Links

Our Site contains affiliate links. If you click on an affiliate link and make a purchase, we may earn a commission at no additional cost to you. We only recommend products and services we genuinely use and trust.

**Affiliate Disclaimer**: Some links are affiliate links, meaning we earn a commission at no extra cost to you if you purchase through them.

## Changes to This Privacy Policy

We may update this Privacy Policy from time to time. Changes will be posted on this page with an updated "Last Updated" date. We encourage you to review this Privacy Policy periodically.

Material changes will be notified via:
- Prominent notice on our Site
- Email notification (if you're subscribed)

## Contact Us

If you have questions about this Privacy Policy or our privacy practices, please contact us:

**Email**: stellaacharoiro@gmail.com
**Website**: [acharoiro.com](https://acharoiro.com)

## Governing Law

This Privacy Policy is governed by the laws of Kenya, without regard to its conflict of law provisions.

---

*This privacy policy was last updated on November 17, 2025.*
